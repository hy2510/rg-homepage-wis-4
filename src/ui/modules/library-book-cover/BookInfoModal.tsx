...
export function BookInfoModal({
  ...
  const onClickStartStudy = () => {
    // MEMO: Mobile 환경에서 Dodo ABC 못하게 막음.
    const isMobileDodoBlock =
      process.env.NEXT_PUBLIC_BETA_MOBILE_DODO_BLOCK === 'Y'
    if (isMobileDodoBlock && isMobile) {
      const isPK = bookInfo.bookCode.includes('EB-PK')
      if (isPK) {
        const bookRound = Number(bookInfo.bookCode.split('-')[2])
        if (bookRound >= 300) {
          alert(process.env.NEXT_PUBLIC_BETA_MSG_OPEN_TO_BE)
          return
        }
      }
    }
    // MEMO: 에듀템 서비스 연동 전까지 SightWords1,2 못하게 막음.
    const isBetaSpeak = process.env.NEXT_PUBLIC_BETA_SPEAK_LOADING === 'Y'
    if (isBetaSpeak) {
      const isPK = bookInfo.bookCode.includes('EB-PK')
      if (isPK) {
        const bookRound = Number(bookInfo.bookCode.split('-')[2])
        if (bookRound >= 500 && bookRound <= 560) {
          alert(process.env.NEXT_PUBLIC_BETA_MSG_OPEN_TO_BE)
          return
        }
      }
  ...
  const onClickStartSpeak = () => {
    // MEMO: 에듀템 서비스 연동 전까지 SPEAK 못하게 막음.
    const isBetaSpeak = process.env.NEXT_PUBLIC_BETA_SPEAK_LOADING === 'Y'
    if (isBetaSpeak) {
      alert(process.env.NEXT_PUBLIC_BETA_MSG_OPEN_TO_BE)
      return
