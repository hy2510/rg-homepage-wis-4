...
const TitleBox = ({txt1, txt2, txt1Color, txt2Color}: {txt1: string, txt2: string, txt1Color? :string, txt2Color?: string}) => {
  const style = useStyle(STYLE_ID)

  return (
    <>
      <div className={style.title_box}>
        <div className={style.txt_1} style={{color: txt1Color}}>{txt1}</div>
        <div className={style.txt_2} style={{color: txt2Color}}>{txt2}</div>
      </div>
    </>
  )
}
  ...
  return (
    <>
      <div className={style.card_title}>
          <div className={style.txt_1} dangerouslySetInnerHTML={{ __html: txt1 }}></div>
          <div className={style.txt_2} dangerouslySetInnerHTML={{ __html: txt2 }}></div>
        </div>
    </>
  )
}
  ...
  return (
    <>
      <div className={style.slide_container}>
        <div className={style.slide_num}>{`${slideNumber + 1} / ${cardData.length}`}</div>
        <div className={`${style.left_arrow} ${theme == 'red' ? style.red : ''}`} onClick={() => {
          0 == slideNumber ? setSlideNumber(cardData.length - 1) : setSlideNumber(slideNumber - 1);
        }}></div>
  ...
  const slideCardData = [
    {
      ...
      subMobile: '재미있는 영상과 5단계 액티비티, 신나는 동요와 복습 게임으로 알파벳, 파닉스, 사이트 워드를 배워요.',
    ...
    },
    {
      ...
      subMobile: '오디오를 포함한 eBook, 애니메이션으로 감상하는 Movie Book, 영어로 된 원서 pBook 등 6,000여 개 이상의 다양한 콘텐츠를 경험해요. ',
    ...
    },
    {
      ...
      subMobile: 'AI 음성인식 프로그램이 eBook 속 원어민 음성을 따라 읽으면 음소 단위로 분석해서 결과를 알려줘요.',
      imgSrc: '/src/images/@about-to-school/section01/slide/3.png',
    },
    {
      title: '꼼꼼한 5단계 독후 학습',
      sub: '레벨별 수준에 맞는 적절한 학습으로<br /> 영어의 4대 영역을 고르게 키워요.',
      subMobile: '레벨별 수준에 맞는 적절한 학습으로 영어의 4대 영역을 고르게 키워요.',
    ...
    <>
      <div className={style.section01}>
        <div className={style.title_image}>
          <Image src={'/src/images/@about-to-school/section01/top_title.svg'} width={600} height={205} alt='' />
        </div>
        <div className={style.contents}>
          <div>리딩게이트는 우리나라 아이들의 영어 학습을 위해 설계된{isMobile ? '' : <br />} 영어 독서 프로그램으로{isMobile ? '' : <br />} 수준별 독서와 꼼꼼한 독후 학습을 제공합니다.</div>
  ...
  return (
    <>
      <div className={style.section02}>
        <TitleBox txt1={'22단계의 세분화된'} txt2={'레벨구성'} txt1Color={''} txt2Color={'#F95977'} />
        <div className={style.row}>
          <div className={style.table_image}>
              <Image src={'/src/images/@about-to-school/section02/table.png'} width={918} height={130} alt='' />
          </div>
        </div>
        <div className={style.row}>
          <SlideContainer theme='red' slideCardData={slideCardPkData}></SlideContainer>
        </div>
        <div className={style.row}>
          <SlideContainer theme='red' slideCardData={slideCardKData}></SlideContainer>
        </div>
        <div className={style.row}>
          <SlideContainer theme='red' slideCardData={slideCard1Data}></SlideContainer>
        </div>
        <div className={style.row}>
          <SlideContainer theme='red' slideCardData={slideCard2Data}></SlideContainer>
        </div>
        <div className={style.row}>
          <SlideContainer theme='red' slideCardData={slideCard36Data}></SlideContainer>
        </div>
      </div>
    </>
  )
  ...
  return (
    <>
      <div className={style.section03}>
        <div className={style.row}>
          <TitleBox txt1={'레벨별 수준에 딱 맞는'} txt2={'독후 학습'} txt1Color={''} txt2Color={'var(--blue)'} />
          <div className={style.comment}>독서 후 4~5단계의(Reading Comprehension / Vocabulary / Listening / Summary / Writing)<br />학습을 통해 학습 성취도를 확인할 수 있습니다.</div>
        </div>
        <div className={style.row}>
          <SlideContainer theme='' slideCardData={slideCardKData}></SlideContainer>
        </div>
        <div className={style.row}>
          <SlideContainer theme='' slideCardData={slideCard1Data}></SlideContainer>
        </div>
        <div className={style.row}>
          <SlideContainer theme='' slideCardData={slideCard2Data}></SlideContainer>
        </div>
      </div>
    </>
  )
  ...
  return (
    <>
      <div className={style.section04}>
        <div className={style.row}>
          <TitleBox txt1={'음성인식 프로그램'} txt2={'AI SPEAK'} txt1Color={''} txt2Color={'#F95977'} />
          <div className={style.comment}>DODO ABC Sight Word 학습과 eBook K~1레벨에서 Speak 프로그램을 이용할 수 있습니다.<br />학생의 발음을 음소 단위로 분석하여 결과를 제공함으로써 스피킹 능력을 향상시킵니다.</div>
        </div>
        <div className={style.row}>
          <SlideContainer theme='red' slideCardData={slideCardPkData}></SlideContainer>
        </div>
  ...
  return (
    <>
      <div className={style.section05}>
        <div className={style.row}>
          <TitleBox txt1={'학습에 재미를 더해주는'} txt2={'포인트와 동기 부여 기능'} txt1Color={''} txt2Color={'var(--blue)'} />
          <div className={style.comment}>
            <div className={style.col_left}>
              <Image src='/src/images/@about-to-school/section05/book.png' width={141} height={220} alt='' />
            </div>
            <div className={style.col_right}>
              <div>리딩게이트의 모든 도서는 단어와 페이지 수, 문장의 난이도를 고려한 포인트가 차등 부여되어 있습니다.</div>
              <div>온라인 독후 학습을 평균 70점 이상으로 통과할 경우 해당 도서의 포인트를 획득할 수 있습니다.</div>
              <div>포인트는 도서 당 최대 두 번까지 획득할 수 있으며, 첫 번째 학습 통과 시 100%, 두 번째 학습 통과 시 50% 포인트를 획득합니다.</div>
            </div>
          </div>
        </div>
        <div className={style.row}>
          <SlideContainer theme='' slideCardData={slideCardRow1Data}></SlideContainer>
        </div>
        <div className={style.row}>
          <div className={style.end_image}>
            <Image src={'/src/images/@about-to-school/section05/ending@2x.png'} width={1768} height={504} alt='' />
          </div>
        </div>
      </div>
    </>
